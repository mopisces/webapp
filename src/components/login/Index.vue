<template>
	<div>
		...
		<template v-if="config.selectShow">
			<van-cell-group title="请选择">
				<van-cell title="我是客户" 	is-link @click="selectType( 'client' )"/>
				<van-cell title="我是管理员" is-link @click="selectType( 'staff' )"/>
			</van-cell-group>
		</template>
		<template v-else>
				...
				this.$request.wechat.config.getBindInfo().then((res)=>{
					if( res.errorCode == '00000' ){
						...
						if( self.formData.stopped == 0 ){
							...
							if( self.config.userInfo.length > 1 ){
