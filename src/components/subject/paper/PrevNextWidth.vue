		...
		<van-popup  v-model="config.popup.show" position="top" :style="{ height: '100%' }">
			...
			<div class="content" style="width:100%;margin-top:46px;">
				<van-radio-group v-model="radio" v-if="config.popup.show">
					<van-cell-group>
						<div role="button" tabindex="0" class="van-cell van-cell--clickable" v-for="(item,index) in radioData" :key="index" @click="radioClick( item.PaperWidth,index )">
							...
							<van-radio slot="right-icon" :name="item.PaperWidth" />
						</div>
					</van-cell-group>
				</van-radio-group>
...
<script>
	...
	export default {
		...
		methods:{
			controllerPrevNext(){
				...
				if( this.radioData[ prev ] == undefined ){
					...
					this.config.header.prev.data = this.radioData[ prev ].PaperWidth;
				}
				if( this.radioData[ next ] == undefined ){
					...
					this.config.header.next.data = this.radioData[ next ].PaperWidth;
				}
				if( this.radioData[ this.config.header.index ] == undefined ){
					...
					this.config.header.middle.data = this.radioData[ this.config.header.index ].PaperWidth;
				}
			},
			...
			headerChange( type ){
				...
				this.radio = this.radioData[ this.config.header.index ].PaperWidth;
				this.controllerPrevNext();
			},
			middleClick(){
				this.radio = this.radioData[ this.config.header.index ].PaperWidth;
		...
		created(){
			if( this.radioData[0].PaperWidth ){
				this.radio = this.radioData[0].PaperWidth;
			}
		},
		computed:{
			radioChange(){
